...
export default function formDataToObject(formData: FormData): any {
	...
	formData.forEach((value, path) => {
		...
		// Do some rudimentary processing of the value in case it's empty, null,
		// a bool or a number
		let newValue = processValue(value);

		// If there's a dot or a bracket in the path, we need to get the
		// (possibly nested or in an array) target
		if (path.match(nestedRegex)) {
			const parts = path
				// NOTE: Use & to indicate an automatic array, because there
				// shouldn't be any properties with that name?
				.replace(/\[\]/g, "[&]")
...
function getTarget(target: any, part: string, isArray: boolean) {
	// If this is our automatic array indicator, just push a new item into the
	// array and return it
	if (part === "&") {
		target.push(isArray ? [] : {});
	...
	// Make sure the target property exists, and set it to the default
	// array/object value
...
function setValue(target: any, key: string, value: any) {
	...
	if (Array.isArray(target)) {
		// If this is our automatic array indicator, just push a new item into
		// the array
		if (key === "&") {
			target.push(value);
		} else {
