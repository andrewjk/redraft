...
export default {
	...
	actions: {
		...
		likePost: async ({ appData, request }) => {
			const user = appData.user;
			if (!user) {
				return unauthorized();
			}
			...
			const result = await api.post(`feed/like`, post, user.token);
			if (result.errors) {
				return unprocessable(result);
			}
		},
		savePost: async ({ appData, request }) => {
			const user = appData.user;
			if (!user) {
				return unauthorized();
			}
			...
			const result = await api.post(`feed/save`, post, user.token);
			if (result.errors) {
				return unprocessable(result);
			}
		},
